...
//import { EvaluatorPointer, baseFunctionTable } from "./Evaluator.js";
"use strict";
const externalFunctionDefinitions = {
    'sum': {
        ev: [false, true, true, false],
        func: function (variable, start, end, expr, that) {
            if (!start.im.eq(0))
                throw new Error("complex number sum index");
            if (!end.im.eq(0))
                throw new Error("complex number sum index");
    ...
    'prod': {
        ...
        func: function (variable, start, end, expr, that) {
            if (!start.im.eq(0))
                throw new Error("complex number prod index");
            if (!end.im.eq(0))
                throw new Error("complex number prod index");
            var result = ComplexDecimal.one();
    ...
    'plot': {
        ...
        func: function (expr, variable, minx, maxx, that) {
            insertOutput = 'plot';
            if (!minx.im.eq(0)) {
