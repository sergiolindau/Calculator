interface ShellInterface {
    ...
    promptArray: Array<any>;
    promptSet: any;
    promptPointer: number;
    batchResize: Function;
    batchDelayedResize: Function;
    promptFocus: Function;
    promptBlur: Function;
    promptKeydown: Function;
    promptCreate: Function;
    promptAppend: Function;
...
const Shell: ShellInterface = {
    ...
    promptSet: {},
    promptPointer: -1,
    batchResize: function(event: Event){
        Shell.batcharea.style.height = '1em';
        Shell.batcharea.style.height = Shell.batcharea.scrollHeight+27+'px';
    },
    batchDelayedResize: function(event: Event){
        window.setTimeout(Shell.batchResize, 0);
    },
    promptFocus: function(event: Event){
		Shell.promptPointer = Shell.promptArray.indexOf(document.activeElement?.id.substring(1));
    },
    promptBlur: function(event: Event){
    ...
    promptKeydown: function(event: KeyboardEvent) {
		var onfocus = document.activeElement as HTMLInputElement;
		if (!event.ctrlKey && !event.altKey && !event.metaKey) {
			if ((event.keyCode === 13 || event.which === 13) && !event.shiftKey) {
				...
				if (onfocus.selectionStart==0) {
					...
					var pdiv = document.getElementById(("d"+onfocus?.id.substring(1)));
					var uid = $.uid();
					...
					if ((Shell.promptPointer+1)==Shell.promptArray.length) {
						...
						Shell.linesArray.push(Shell.promptSet[onfocus?.id.substring(1)][2].value);
						Shell.batcharea.value = Shell.linesArray.join("\n");
						Shell.promptPointer++;
					}
          ...
          onfocus.selectionStart = onfocus.value.length;
				...
				if (Shell.promptPointer>0) Shell.promptSet[Shell.promptArray[Shell.promptArray.indexOf(onfocus.id.substring(1))-1]][2].focus();
			}
			else if ((event.keyCode === 40 || event.which === 40)){
				if ((Shell.promptPointer+1)<Shell.promptArray.length) Shell.promptSet[Shell.promptArray[Shell.promptArray.indexOf(onfocus.id.substring(1))+1]][2].focus();
			}
		}
    },
    promptCreate: function(uid: string, div: HTMLDivElement) {
    ...
    promptAppend: function(text: string | null) {
		var uid = $.uid();
		var div = $.create("div",Shell.thisprompt,"d"+uid);
		Shell.promptCreate(uid,div);
		Shell.promptPointer++;
		Shell.promptArray.push(uid);
		if (text) Shell.promptSet[uid][2].value=text;
    ...
    loadLines: function() {
		if (Shell.linesArray && (Shell.linesArray.length==0)) Shell.linesArray=[""];
		if (Shell.linesArray) {
    ...
    batchFocus: function(event: Event) {
		var onfocus = document.activeElement;
    },
    batchExec: function(event: Event) {
		Shell.promptClean();
		Shell.loadBatch();
		Shell.loadLines();
		Shell.batchbutton.focus();
    },
    batchBlur: function(event: Event) {
        Shell.promptClean();
    },
    init: function(panel: string, func: Function, linesArray: Array<string>){
        Shell.thispanel = $.i(panel);
        if (func) {
            Shell.evalPrompt = func;
        }
        else {
            Shell.evalPrompt = function(div: HTMLDivElement,pre: HTMLPreElement,input: HTMLInputElement,output: HTMLDivElement){console.log("evalPrompt("+input.value+")");};
        }
