...
const Send = {
    ...
    buildMessageBody: function () {
        return ("# Nome: " + Send.input_name.value.trim() + "\n" +
            "# Turma: " + Send.input_class.value.trim() + "\n" +
            "# Data: " + (new Date()).toLocaleDateString() + " " + (new Date()).toLocaleTimeString() + "\n\n" +
            Send.linesArray.join('\n'));
    },
    ...
    sendEmailMessage: function () {
        ...
        let target = `mailto:${escfunc(Send.addressList[Send.menu_sendto.selectedIndex][0].substring(0, Send.addressList[Send.menu_sendto.selectedIndex][0].indexOf(',')))} `;
        target += `${escfunc("<" + Send.addressList[Send.menu_sendto.selectedIndex][1] + ">")}?`;
        target += `subject=${escfunc("[" + Send.input_name.value.trim() + " - " + Send.input_class.value.trim() + "] " + (new Date()).toLocaleDateString() + " " + (new Date()).toLocaleTimeString())}&`;
        target += `body=${escfunc(Send.buildMessageBody())}`;
        Send.openTarget(target);
    },
    sendMessage: function (event) {
        if ((Send.input_name.value.trim() !== "") && (Send.input_class.value.trim() !== "")) {
            if (Send.isphone()) {
