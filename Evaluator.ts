 ...
 */
type TBaseClass = {[k: string]: string};
type TFunctionList = Array<string>;
type TFunctionListTable = {[k: string]: TFunctionList};
...
type TBaseFunctionTable = {[k: string]: TBaseFunctionTableEntry};

/**
 * Abstract Syntax Tree nodes
 */
type NodeExpr = NodeName | NodeArgExpr | NodeOperation | NodeList;
 ...
 * Language keywords
 */
 const keywordsTable: Array<string> = [
	'global',
	'persistent',
	'if',
	'endif',
 ...
 * Word-list commands
 */
const commandsTable: Array<string> = [
	'help',
	'doc'
];
 ...
 */
var baseFunctionTable: TBaseFunctionTable = {};
 
 /**
 * External parser declarations
 */
declare var parser: any;				// parser made by Jison
declare var EvaluatorPointer: any;		// Window.EvaluatorPointer defined in parser body
...
class Evaluator {
	...
	debug: 				boolean;
	nativeNameTable: 	{[k: string]: any};
	nameTable: 			{[k: string]: any};
	localTable: 		{[k: string]: any};
	parser: 			any;
	exitStatus:			number;
	exitMessage:		string;
	...
	opTable:			TFunctionTable;
	aliasTable:			TAliasNameTable;
	nodeBoolean: 		Function;
	isBoolean: 			Function;
	unparseBoolean: 	Function;
	unparseBooleanML: 	Function;
	nodeString: 		Function;
	isString: 			Function;
	...
	DefUnOpFunction(name: string) {
		...
		baseFunctionTable[name] = {
			mapper: false,
			func:result_function as unknown as Function
		}
	}

	DefBinOpFunction(name: string) {
		let result_function;
	...
	DefBinMoreOpFunction(name: string) {
		let result_function;
		eval("result_function=(left,...right) => {\n\
			if (right.length==1) {\n\
				return Tensor."+name+"(left,right[0]);\n\
	...
	defFunction(name: string, obj: string, map?: boolean) {
		baseFunctionTable[name] = {
	...
	Evaluator(tree: any,local: boolean,fname:string): any {
		...
		if (this.debug) console.log("Evaluator(tree:"+JSON.stringify(tree)+",local:"+local+",fname:"+fname+")");
		if ('list' in tree) {
					...
					if (aliasTreeName in baseFunctionTable) { // Is base function
						if (typeof baseFunctionTable[aliasTreeName]['mapper']!=='undefined') {  // arguments evaluated
							...
							for (let i=0;i<tree.args.length;i++) { // Evaluate arguments list selectively
								argumentsList[i] = (baseFunctionTable[aliasTreeName] as any).ev[i]?this.Evaluator(tree.args[i],local,fname):tree.args[i];
							}
...
export {
	Evaluator,TEvaluatorConfig,EvaluatorPointer,EvaluatorResponse,
	TBaseFunctionTableEntry,TBaseFunctionTable,baseFunctionTable,
	NodeName,NodeArgExpr,NodeOperation,NodeList,NodeExpr,
	keywordsTable
};