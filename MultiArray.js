...
class MultiArray {
    ...
    static evaluate(tree, that, fname) {
    ...
    static isSameDim(left, right) {
        return !(left.dim < right.dim || left.dim > right.dim);
    }
    static number2matrix1x1(value) {
        if ('array' in value) {
            return value;
        }
        else if ('re' in value) {
    ...
    static map(M, f) {
        ...
        for (var i = 0; i < M.dim[0]; i++) {
            result.array[i] = M.array[i].map(f);
        }
        return result;
    }
    static expandRange(startNode, stopNode, strideNode) {
        var temp = new Array();
    ...
    static testIndex(k, bound, matrix, input) {
        if (!k.re.isInteger() || !k.re.gte(1))
            throw new Error(input + ": subscripts must be either integers greater than or equal 1 or logicals");
        if (!k.im.eq(0))
            throw new Error(input + ": subscripts must be real");
    ...
    static subMatrix(temp, id, argumentsList) {
            ...
            for (var i = 0; i < left.dim[0]; i++) {
                ...
                for (var j = 0; j < right.dim[1]; j++) {
                    for (var n = 0; n < left.dim[1]; n++) {
                        result.array[i][j] = ComplexDecimal.add(result.array[i][j], ComplexDecimal.mul(ComplexDecimal.toThis(left.array[i][n]), ComplexDecimal.toThis(right.array[n][j])));
                    }
                }
            }
            return result;
    ...
    static scalarOpMultiArray(op, left, right) {
        ...
        for (var i = 0; i < result.dim[0]; i++) {
            ...
            for (var j = 0; j < result.dim[1]; j++) {
                result.array[i][j] = ComplexDecimal[op](left, ComplexDecimal.toThis(right.array[i][j]));
            }
        }
        return result;
    }
    static MultiArrayOpScalar(op, left, right) {
        var result = new MultiArray(left.dim);
        for (var i = 0; i < result.dim[0]; i++) {
            result.array[i] = new Array(result.dim[1]);
            for (var j = 0; j < result.dim[1]; j++) {
                result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(left.array[i][j]), right);
            }
        }
        return result;
    }
    ...
    static ewiseOp(op, left, right) {
        if (MultiArray.arrayEqual(left.dim.slice(0, 2), right.dim.slice(0, 2))) {
            ...
            for (var i = 0; i < result.dim[0]; i++) {
                result.array[i] = new Array(result.dim[1]);
                for (var j = 0; j < result.dim[1]; j++) {
                    result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(left.array[i][j]), ComplexDecimal.toThis(right.array[i][j]));
                }
            }
            return result;
        }
            ...
            for (var i = 0; i < col.dim[0]; i++) {
                ...
                for (var j = 0; j < matrix.dim[1]; j++) {
                    result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(col.array[i][0]), ComplexDecimal.toThis(matrix.array[i][j]));
                }
            }
            return result;
        }
            ...
            for (var i = 0; i < matrix.dim[0]; i++) {
                ...
                for (var j = 0; j < row.dim[1]; j++) {
                    result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(row.array[0][j]), ComplexDecimal.toThis(matrix.array[i][j]));
                }
            }
            return result;
        }
            ...
            for (var i = 0; i < right.dim[0]; i++) {
                ...
                for (var j = 0; j < left.dim[1]; j++) {
                    result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(left.array[0][j]), ComplexDecimal.toThis(right.array[i][0]));
                }
            }
            return result;
        }
            ...
            for (var i = 0; i < left.dim[0]; i++) {
                ...
                for (var j = 0; j < right.dim[1]; j++) {
                    result.array[i][j] = ComplexDecimal[op](ComplexDecimal.toThis(left.array[i][0]), ComplexDecimal.toThis(right.array[0][j]));
                }
            }
            return result;
        }
    ...
    static eye(i, j) {
        if (!i.re.isInteger())
            throw new Error("Invalid call to eye. Non integer number argument.");
    ...
    static min(M) {
        ...
        var temp = M;
        var result = M.array[0][0];
        if (!M.array[0][0].im.eq(0))
            throw new Error("complex number in min function");
        if (DMin == 1) {
            if (DMin == M.dim[1]) {
                temp = MultiArray.transpose(M);
            }
            for (var i = 1; i < temp.dim[1]; i++) {
                if (!temp.array[0][i].im.eq(0))
                    throw new Error("complex number in min function");
    ...
    static max(M) {
        ...
        var result = M.array[0][0];
        if (!M.array[0][0].im.eq(0))
            throw new Error("complex number in max function");
        if (DMin == 1) {
            if (DMin == M.dim[1]) {
                temp = MultiArray.transpose(M);
            }
    ...
    static mean(M) {
        var DMin = Math.min(M.dim[0], M.dim[1]);
        var temp = M;
        var result = ComplexDecimal.zero();
        if (DMin == 1) {
            if (DMin == M.dim[1]) {
                temp = MultiArray.transpose(M);
            }
    ...
    static gauss(M, x) {
        ...
        for (i = 0; i < n; i++) {
            ...
            var maxEl = ComplexDecimal.abs(A.array[i][i]), maxRow = i;
            for (k = i + 1; k < n; k++) {
